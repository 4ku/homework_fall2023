...
def collect_mbpo_rollout(
    ...
    for _ in range(rollout_len):
        ...
        next_obs = np.array([mb_agent.get_dynamics_predictions(i, ob, ac) for i in range(mb_agent.ensemble_size)])
        next_ob = next_obs.mean(axis=0)
    ...
    for itr in range(config["num_iters"]):
        ...
        for _ in tqdm.trange(
            ...
            for i in range(mb_agent.ensemble_size):
                ...
                mb_agent_loss = mb_agent.update(i, batch["observations"], batch["actions"], batch["next_observations"])
                step_losses.append(mb_agent_loss)
